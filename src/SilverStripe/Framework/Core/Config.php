...
use SilverStripe\Framework\Manifest\ManifestInterface;
...
class Config {
	...
	/**
	 * @deprecated 3.1 Please use {@link Application::getConfig()}
	 */
	public static function inst() {
		return Application::curr()->getConfig();
	}
	...
	public function init() {
		$this->config = $this->manifest->getConfig();

		foreach($this->manifest->getPhpConfigs() as $path) {
			include $path;
		}
	}
	...
	public function forClass($class) {
		if (isset(self::$for_class_instances[$class])) {
			...
			return self::$for_class_instances[$class] = new ConfigForClass($class);
		}
	}
	...
	function get($class, $name, $sourceOptions = 0, &$result = null, $suppress = null) {
		...
		if(isset($this->config[$class][$name])) {
			self::merge_low_into_high($result, $this->config[$class][$name], $suppress);
			if ($result !== null && !is_array($result)) return $result;
		}
