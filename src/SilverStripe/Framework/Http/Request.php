...
class Request extends Message implements \ArrayAccess {
	...
	/**
	 * This code is adapted from the Zend Framework.
	 *
	 * @copyright Copyright (c) 2005-2012 Zend Technologies USA Inc. (http://www.zend.com)
	 * @license http://framework.zend.com/license/new-bsd New BSD License
	 */
	protected function extractRequestUri() {
		$format = function($val) { return ltrim($val, '/'); };
		$requestUri = null;
		...
		if ($requestUri !== null) {
			return $format(preg_replace('#^[^:]+://[^/]+#', '', $requestUri));
		}
	...
	protected function extractBaseUrl() {
		$format = function($val) { return rtrim($val, '/') . '/'; };

		$baseUrl        = '';
		$filename       = realpath($this->serverVar('SCRIPT_FILENAME'));
		$scriptName     = $this->serverVar('SCRIPT_NAME');
		$phpSelf        = $this->serverVar('PHP_SELF');
		$origScriptName = $this->serverVar('ORIG_SCRIPT_NAME');
		...
		if(strpos($filename, BASE_PATH) === 0) {
			$remove = substr($filename, strlen(BASE_PATH));

			if(substr($scriptName, -strlen($remove)) == $remove && $scriptName != $remove) {
		...
		if (0 === strpos($requestUri, $baseUrl)) {
			return $format($baseUrl);
		}

		// Directory portion of base path matches.
		$baseDir = str_replace('\\', '/', dirname($baseUrl));
		if (0 === strpos($requestUri, $baseDir)) {
			return $format($baseDir);
		}
