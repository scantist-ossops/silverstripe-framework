 ...
 *     are defined in {@link FileFinder::$vcs_dirs}.
...
class FileFinder {
	...
	public function __construct() {
		...
		do {
			$options = array_merge($class::$default_options, $options);
	...
	protected function acceptDir($basename, $pathname, $depth) {
		...
		if ($callback = $this->getOption('accept_callback')) {
			if (call_user_func($callback, $basename, $pathname, $depth) === false) return false;
		}
